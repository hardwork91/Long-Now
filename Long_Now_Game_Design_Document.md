...
Version 0.4 (in progress) — Living design document
...
### 13.2 Built So Far
- A first playable HTML prototype of the core loop (real-time + pause, 4 crew with traits as internal modifiers, 4 visible state bars, tasks Sleep/Eat/Garden, O2 as the binding station resource).

### 13.3 Deferred (designed, not yet in prototype)
Multiple locations & UI skins; recruitment; O2-plant/room upgrades; base expansion; events (fire, etc.); exploration; full materials economy; traits shown as visible "+10" bonuses on a recruitment screen; "away/exploring" crew representation; drag-from-crew-card assignment; O2 deficit cascade (gradual fall → whole-crew death at 0%, with a sub-10% critical highlight).
